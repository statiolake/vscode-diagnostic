...
export class Config {
  ...
  public traceServer: "off" | "messages" | "verbose";

  public linters: {
    [linterName: string]: object;
  };
  public lintFiletypes: {
    [filetype: string]: string | string[];
  };

  public formatters: {
    [formatterName: string]: object;
  };
  public formatFiletypes: {
    [filetype: string]: string | string[];
  };

  constructor(config: WorkspaceConfiguration) {
    this.enableDebugging = config.get("debug", false);
    this.traceServer = config.get("trace.server", "off");

    this.linters = {
      ...defaultLinters,
      ...config.get("linters", {})
    };
    this.lintFiletypes = config.get("lintFiletypes", {});

    this.formatters = {
      ...defaultFormatters,
      ...config.get("formatters", {})
    };
    this.formatFiletypes = config.get("formatFiletypes", {});
  }
}

// Those linter settings are retrieved from <https://github.com/iamcco/coc-diagnostic/blob/master/src/config.ts>. Thanks!
export const defaultLinters = {
  eslint: {
  ...
  pylint: {
    ...
    command: "pylint",
    debounce: 500,
    args: ["-f", "json", "--from-stdin", "%filename"],
    parseJson: {
      line: "line",
      column: "column",
      endLine: "endLine",
